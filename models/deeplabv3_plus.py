...
class ResNet(nn.Module):
    def __init__(self, drop_conv=0.0, in_channels=3, output_stride=16, backbone='resnet50', pretrained=True):
        super(ResNet, self).__init__()
        #model = getattr(models, backbone)(pretrained)
        if backbone=='resnet50':
            model = resnet50(drop_conv=drop_conv)
        elif backbone=='resnet101':
            model = resnet101(drop_conv=drop_conv)
        else:
...
class DeepLab(BaseModel):
    def __init__(self, num_classes, drop_conv=0.0, in_channels=3, backbone='resnet50', pretrained=True, 
                output_stride=16, freeze_bn=False,freeze_backbone=False,  **_):
                
        super(DeepLab, self).__init__()
        assert ('xception' or 'resnet' in backbone)
        if 'resnet' in backbone:
            self.backbone = ResNet(in_channels=in_channels, output_stride=output_stride, backbone=backbone, pretrained=pretrained, drop_conv=drop_conv)
